...
Данный репозиторий не обновляется и оставлен как итог работы над дипломной работой. 
Приложение работает, имеются баги. 
Разработка была продолжена в изначальном репозитории, этот является лишь его дампом. 

Актуальная версия находится [здесь](https://github.com/netology-code/bhj-diploma).
...
Работа может быть реализована 2-мя способами: с использованием удалённого сервера по адресу "https://bhj-diplom.letsdocode.ru" 
или развёрнутым локально по адресу "http://localhost:8000". Описание по запуску локального сервера и серверной логики можете найти в [файле](./md/server.md)
